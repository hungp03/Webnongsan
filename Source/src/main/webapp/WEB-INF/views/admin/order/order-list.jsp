												...
												<div class="dropdown">
													...
													<ul class="dropdown-menu">
														<li><a class="dropdown-item"
															href="admin/orders.htm?filter=0">Tất cả</a></li>
														<li><a class="dropdown-item"
															href="admin/orders.htm?filter=1">Chờ xử lý</a></li>
														<li><a class="dropdown-item"
															href="admin/orders.htm?filter=2">Đang giao</a></li>
														<li><a class="dropdown-item"
															href="admin/orders.htm?filter=3">Đã giao</a></li>
														<li><a class="dropdown-item"
															href="admin/orders.htm?filter=4">Hủy</a></li>
													</ul>
												</div>
											</div>
										</div>
											...
											<c:forEach varStatus="status" var="item" items="${orders}">
												<tr>
													<td class="align-middle"><a
														href="admin/orders/order-detail.htm?orderId=${item.orderId }"
													...
													<td class="align-middle"><div class="dropdown">
															<button class="btn btn-sm btn-custom dropdown-toggle"
																type="button" data-bs-toggle="dropdown"
																aria-expanded="false">
																<c:if test="${item.status == 0 }">Chờ xử lý</c:if>
																<c:if test="${item.status == 1 }">Đang giao</c:if>
															...
															<ul class="dropdown-menu">
																<li><a
																	...
																	href="admin/orders/update-order.htm?id=${item.orderId }&status=0">Chờ
																		xử lý</a></li>
																<li><a
																	class="dropdown-item ${item.status == 2 ? 'disabled' : '' }"
																	href="admin/orders/update-order.htm?id=${item.orderId }&status=1">Đang
																		giao</a></li>
																<li><a
																	class="dropdown-item  ${item.status == 3 ? 'disabled' : '' }"
																	href="admin/orders/update-order.htm?id=${item.orderId }&status=2">Đã
																		giao</a></li>
																<li><a
																	class="dropdown-item ${item.status == 2 ? 'disabled' : '' }"
																	href="admin/orders/update-order.htm?id=${item.orderId }&status=3">Đã
																		hủy</a></li>
													...
													<td class="align-middle"><fmt:formatNumber
															value="${item.price }" type="currency" currencySymbol="đ"
							...
							<nav>
								<ul class="pagination d-flex justify-content-center ms-2">
									<li
										...
										href="admin/orders.htm?crrPage=${crrPage - 1}&filter=${filter}">
									...
									<c:forEach var="i" begin="1" end="${paginate.totalPage}"
										...
										<li
											...
											href="admin/orders.htm?crrPage=${in.count}&filter=${filter}">${in.count}</a>
										</li>
									</c:forEach>
									<li
										class="page-item ${(paginate.currentPage == paginate.totalPage) ? 'disabled' : ''}">
										<a class="page-link mx-1 text-body" aria-label="Next"
										href="admin/orders.htm?crrPage=${paginate.currentPage + 1}&filter=${filter}">
